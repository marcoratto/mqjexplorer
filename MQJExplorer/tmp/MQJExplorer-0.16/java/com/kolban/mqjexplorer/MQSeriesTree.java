...
public class MQSeriesTree implements TreeSelectionListener, MouseListener,ActionListener, Serializable {
	
	private final static Logger logger = Logger.getLogger(MQSeriesTree.class);
	...
	class MQTreeModel extends DefaultTreeModel {
		...
		public int getChildCount(Object obj) {
			...
			logger.info(obj.toString() + " Child Count: " + i);
			return i;
		}
		...
		public boolean isLeaf(Object obj) {
			...
			if (mqjexplorernode1.getType() != 13
					&& mqjexplorernode1.getType() != 12) {
				boolean flag = super.isLeaf(obj);
				return flag;
			} else {
				boolean flag1 = mqjexplorernode1.getJNDINameSpace()
						.getSubContextCount() == 0;
				logger.info(mqjexplorernode.toString()
						+ "->IsLeaf (JMS): " + flag1);
				return flag1;
			}
		}
	...
	class MQTreeExpansionListener implements TreeExpansionListener {

		public void treeExpanded(TreeExpansionEvent treeexpansionevent) {
			logger.info("Path: "
					+ treeexpansionevent.getPath().toString());
			addChildren((MQJExplorerNode) treeexpansionevent.getPath()
			...
			if (obj instanceof MQJExplorerNode) {
				...
				if (mqjexplorernode.getType() == 2)
					if (mqjexplorernode.queueManagerModel.isConnected())
						imageicon = new ImageIcon(getClass().getResource(
								"/icons/queueManagerConnected.gif"));
					else
						imageicon = new ImageIcon(getClass().getResource(
								"/icons/queueManager.gif"));
				if (mqjexplorernode.getType() == 12)
					if (mqjexplorernode.getJNDINameSpace().isConnected())
						imageicon = new ImageIcon(getClass().getResource(
								"/icons/treeicons/HomeConnected.gif"));
					else
						imageicon = new ImageIcon(getClass().getResource(
								"/icons/treeicons/HomeDisconnected.gif"));
				if (mqjexplorernode.getType() == 14
						&& mqjexplorernode.getFlags() == 0)
					imageicon = new ImageIcon(getClass().getResource(
							"/icons/treeicons/closedFolder.gif"));
			}
			setOpenIcon(imageicon);
			setLeafIcon(imageicon);
			setClosedIcon(imageicon);
			return super.getTreeCellRendererComponent(jtree, obj, flag, flag1,
					flag2, i, flag3);
		}
	...
	class MQJExplorerNode extends DefaultMutableTreeNode implements Comparable<MQJExplorerNode> {
		...
		public MQJExplorerNode(String s, QueueManagerModel queuemanagermodel,
				int i) {
			super(s);
		...
		public int compareTo(MQJExplorerNode o) {
			return this.text.compareTo(o.text);
		}
	}

	public MQSeriesTree() {
		treeListeners = new Vector(0, 1);
		lastQueueManagerModel = null;
		queueManagerCount = 0;
		queueManagerVector = new Vector<QueueManagerModel>();
	 ...
	 */
	public void sort() {
		Collections.sort(this.queueManagerVector);
	}
	...
	public void actionPerformed(ActionEvent actionevent) {
		...
		if (s.equals("eventMonitor")) {
			...
			MQJExplorerNode mqjexplorernode2 = (MQJExplorerNode) treepath
					.getLastPathComponent();
			QueueManagerModel queuemanagermodel1 = mqjexplorernode2
					.getQueueManagerModel();
			try {
				eventdlg.setQueueManager(queuemanagermodel1
						.getNewQueueManager());
				eventdlg.pack();
		...
		if (s.equals("mqscCommand")) {
			...
			if (treepath1 == null) {
				...
				MQJExplorerNode mqjexplorernode3 = (MQJExplorerNode) treepath1
						.getLastPathComponent();
				escapedlg.setQueueManagerModel(mqjexplorernode3
						.getQueueManagerModel());
				escapedlg.pack();
		...
		if (s.equals("qmgrProperties")) {
			...
			if (treepath2 == null) {
				...
				MQJExplorerNode mqjexplorernode = (MQJExplorerNode) treepath2
						.getLastPathComponent();
		...
		if (s.equals("qmgrStop")) {
			...
			if (treepath3 == null) {
				...
				MQJExplorerNode mqjexplorernode1 = (MQJExplorerNode) treepath3
						.getLastPathComponent();
				QueueManagerModel queuemanagermodel = mqjexplorernode1
						.getQueueManagerModel();
				queuemanagermodel.disconnect();
				StopDialog stopdialog = new StopDialog();
				stopdialog.setQueueManagerName(queuemanagermodel
						.getQueueManagerName());
				stopdialog.pack();
	...
	private void addChildren(MQJExplorerNode mqjexplorernode) {
		for (Enumeration enumeration = mqjexplorernode.children(); enumeration
				.hasMoreElements();) {
			MQJExplorerNode mqjexplorernode1 = (MQJExplorerNode) enumeration
					.nextElement();
			if ((mqjexplorernode1.getType() == 13 || mqjexplorernode1.getType() == 12)
					& (!mqjexplorernode1.getVisited())) {
				JNDINameSpace jndinamespace = mqjexplorernode1
						.getJNDINameSpace();
				if (jndinamespace.isConnected()) {
					...
					for (int j = 0; j < i; j++) {
						...
						MQJExplorerNode mqjexplorernode2 = new MQJExplorerNode(
								jndinamespace1, 13);
						treeModel.insertNodeInto(mqjexplorernode2,
								mqjexplorernode1,
								mqjexplorernode1.getChildCount());
					}
	...
	public void addJNDINameSpace(JNDINameSpace jndinamespace) {
		if (jndinamespace == null) {
			...
			jndinamespace.setInitialContext(selectnamespacedlg
		...
		treeModel.insertNodeInto(mqjexplorernode, jmsRootNode,
				jmsRootNode.getChildCount());
	}

	public void addMQSeriesTreeListener(
	...
	public void addQueueManager(QueueManagerModel queuemanagermodel) {
		...
		Collections.sort(queueManagerVector);
	}
	...
	public JTree buildTree() {
		...
		for (Enumeration enumeration1 = vector.elements(); enumeration1
				.hasMoreElements(); addJNDINameSpace(jndinamespace))
	...
	public void connectSelected() {
		...
		if (mqjexplorernode.getType() == 2)
			try {
				...
				QueueManagerModel.displayError(e);
			}
		else if (mqjexplorernode.getType() == 12) {
	...
	private void createSubContext() {
		...
		if (mqjexplorernode.getType() == 13 || mqjexplorernode.getType() == 12) {
			...
			if (newsubcontext.getRc() == 1) {
				JNDINameSpace jndinamespace = mqjexplorernode
						.getJNDINameSpace();
				try {
					JNDINameSpace jndinamespace1 = jndinamespace
							.newSubContext(newsubcontext.getSubContextName());
					jndinamespace.refresh();
					MQJExplorerNode mqjexplorernode1 = new MQJExplorerNode(
							jndinamespace1, 13);
					mqjexplorernode1.setVisited(true);
					treeModel.insertNodeInto(mqjexplorernode1, mqjexplorernode,
							mqjexplorernode.getChildCount());
				} catch (NameAlreadyBoundException _ex) {
					JOptionPane.showMessageDialog(null, "Name already bound",
							"Create Error", 0);
				} catch (Exception exception) {
					JOptionPane.showMessageDialog(null, exception.toString(),
							"Create Error", 0);
					logger.info("Failed to create context"
							+ exception.toString());
				}
			}
		}
	...
	private void deleteSubContext() {
		logger.trace("deleteSubContext()");
		TreePath treepath = tree.getSelectionPath();
		if (treepath == null)
			return;
		MQJExplorerNode mqjexplorernode = (MQJExplorerNode) treepath
				.getLastPathComponent();
		if (mqjexplorernode.getType() == 13) {
			MQJExplorerNode mqjexplorernode1 = (MQJExplorerNode) mqjexplorernode
					.getParent();
			JNDINameSpace jndinamespace = mqjexplorernode1.getJNDINameSpace();
			try {
				jndinamespace.deleteSubContext(mqjexplorernode
						.getJNDINameSpace().getProviderURL());
				jndinamespace.refresh();
				treeModel.removeNodeFromParent(mqjexplorernode);
			} catch (ContextNotEmptyException _ex) {
				JOptionPane.showMessageDialog(null, "Context not empty!",
						"Delete Error", 0);
			} catch (NamingException namingexception) {
				JOptionPane.showMessageDialog(null, namingexception.toString(),
						"Delete Error", 0);
			}
		}
	}
	...
	public void disconnectSelected() {
		logger.trace("disconnectSelected()");
		...
		} else if (mqjexplorernode.getType() == 13 || 
					mqjexplorernode.getType() == 12) {
		...
		do
			try {
				DefaultMutableTreeNode defaultmutabletreenode = (DefaultMutableTreeNode) mqjexplorernode
						.getFirstChild();
	...
	private DefaultMutableTreeNode findQueueManager(
			QueueManagerModel queuemanagermodel) {
		for (Enumeration enumeration = queueManagersNode.children(); enumeration
				.hasMoreElements();) {
	...
	public void forwardMenuAction(ActionEvent actionevent) {
		MQSeriesTreeListener mqseriestreelistener;
		for (Enumeration enumeration = treeListeners.elements(); enumeration
				.hasMoreElements(); mqseriestreelistener
				.menuAction(actionevent))
			mqseriestreelistener = (MQSeriesTreeListener) enumeration
					.nextElement();
	...
	public void load(String s) {
		...
		try {
			...
			do {
				QueueManagerModel queuemanagermodel = (QueueManagerModel) objectinputstream
						.readObject();
	...
	public void maybePopup(MouseEvent mouseevent) {
		...
		if (treepath == null)
			return;
		logger.info("Path: " + treepath.toString());
		...
		case 4: // '\004'
			String as[][] = {
					...
					{ "New Server Connection Channel",
							"createServerConnectionChannel" },
					{ "New Cluster Sender Channel",
							"createClusterSenderChannel" },
					{ "New Cluster Receiver Channel",
							"createClusterReceiverChannel" } };
		...
		case 7: // '\007'
			String as1[][] = { { "New Process Definition", "createProcess" },
					{ "sep", "" }, { "Refresh", "refresh" } };
		...
		case 8: // '\b'
			String as2[][] = { { "New NameList Definition", "createNameList" },
					{ "sep", "" }, { "Refresh", "refresh" } };
		...
		jpopupmenu.show(mouseevent.getComponent(), mouseevent.getX(),
				mouseevent.getY());
	}

	public void modifySelectedQmgr() {
		logger.trace("modifySelectedQmgr()");
		TreePath treepath = tree.getSelectionPath();
		if (treepath == null) {
			return;
		}
		...
		boolean flag = this.selectQueueManager(mqjexplorernode.getQueueManagerModel());
	...
	public void refreshConnected(QueueManagerModel queuemanagermodel) {
		...
		treeModel.insertNodeInto(mqjexplorernode1, mqjexplorernode,
				mqjexplorernode.getChildCount());
		mqjexplorernode1 = new MQJExplorerNode("Client Connections",
				queuemanagermodel, 6);
		treeModel.insertNodeInto(mqjexplorernode1, mqjexplorernode,
				mqjexplorernode.getChildCount());
		mqjexplorernode1 = new MQJExplorerNode("Process Definitions",
				queuemanagermodel, 7);
		treeModel.insertNodeInto(mqjexplorernode1, mqjexplorernode,
				mqjexplorernode.getChildCount());
		mqjexplorernode1 = new MQJExplorerNode("Namelists", queuemanagermodel,
				8);
		treeModel.insertNodeInto(mqjexplorernode1, mqjexplorernode,
				mqjexplorernode.getChildCount());
		if (mqjexplorernode.queueManagerModel.hasPubSub()) {
			MQJExplorerNode mqjexplorernode2 = new MQJExplorerNode("Pub/Sub",
					queuemanagermodel, 14);
			mqjexplorernode2.setFlags(1);
			treeModel.insertNodeInto(mqjexplorernode2, mqjexplorernode,
					mqjexplorernode.getChildCount());
		}
	}
	...
	public void removeSelectedQueueManager() {
		...
		for (Enumeration enumeration = treeListeners.elements(); enumeration
				.hasMoreElements(); mqseriestreelistener
				.disconnectQueueManager(mqjexplorernode.getQueueManagerModel()))
			mqseriestreelistener = (MQSeriesTreeListener) enumeration
					.nextElement();

		try {
			try {
				mqjexplorernode.getQueueManagerModel().getQueueManager()
						.disconnect();
			} catch (Exception _ex) {
			}
	...
	public void save(String s) {
		try {
			...
			QueueManagerModel queuemanagermodel;
			for (Enumeration enumeration = queueManagersNode.children(); enumeration
					.hasMoreElements(); objectoutputstream
					.writeObject(queuemanagermodel))
				queuemanagermodel = (QueueManagerModel) ((DefaultMutableTreeNode) enumeration
	...
	public boolean selectQueueManager(QueueManagerModel qmm) {
		
		ShowQueueManager showqueuemanager = new ShowQueueManager(MQJExplorer.mainFrame);
		showqueuemanager.pack();
		if (qmm.getQueueManagerName() != null) {
			showqueuemanager.setLocal(qmm.isLocal());
			showqueuemanager.setQueueManagerName(qmm.getQueueManagerName());
			showqueuemanager.setConnectionName(qmm.getHostName() + "(" + qmm.getPort() + ")");
			showqueuemanager.setChannelName(qmm.getChannel());
			showqueuemanager.setQueueManagerType(qmm.getQueueManagerType());
			showqueuemanager.setUsername(qmm.getUsername());
			showqueuemanager.setPassword(qmm.getPassword());
			showqueuemanager.setSecurityExit(qmm.getSecurityExit());
		}
		...
		if (showqueuemanager.isLocal()) {
			...
			queuemanagermodel.setUsername(showqueuemanager.getUsername());
			queuemanagermodel.setPassword(showqueuemanager.getPassword());
			queuemanagermodel.setSecurityExit(showqueuemanager.getSecurityExit());
			try {
	...
	public void valueChanged(TreeSelectionEvent treeselectionevent) {
		...
		if (mqjexplorernode.getType() == 13 || mqjexplorernode.getType() == 12) {
			...
			QueueManagerModel queuemanagermodel = mqjexplorernode
					.getQueueManagerModel();
		...
		if (mqjexplorernode.getType() == 14)
			try {
				MQQueue mqqueue = mqjexplorernode.getQueueManagerModel()
						.getQueueManager()
						.accessQueue("SYSTEM.BROKER.CONTROL.QUEUE", 32);
				int j = mqqueue.getOpenInputCount();
				mqqueue.close();
				if (j == 0) {
					logger.info("Queue open count wrong!");
		...
		MQSeriesTreeListener mqseriestreelistener;
		for (Enumeration enumeration = treeListeners.elements(); enumeration
				.hasMoreElements(); mqseriestreelistener.treeSelection(obj, i))
			mqseriestreelistener = (MQSeriesTreeListener) enumeration
					.nextElement();
	...
	private Vector<QueueManagerModel> queueManagerVector;
