...
public class ShowQueueManager extends JDialog {
	...
	private void connEtoM9(ItemEvent itemevent) {
		try {
			...
			getLabelSecurityExit().setEnabled(getShowRemoteRadioButton().isSelected());
			getTextSecurityExit().setEnabled(getShowRemoteRadioButton().isSelected());
		} catch (Throwable throwable) {
			handleException(throwable);
		}
	}
	...
	private JLabel getLabelSecurityExit() {
		if (labelSecurityExit == null)
			try {
				labelSecurityExit = new JLabel();
				labelSecurityExit.setName("labelSecurityExit");
				labelSecurityExit.setText("Security Exit:");
				labelSecurityExit.setEnabled(false);
			} catch (Throwable throwable) {
				handleException(throwable);
			}
		return labelSecurityExit;
	}
	...
	private JPanel getJPanel1() {
		if (ivjJPanel1 == null)
			try {
				...
				getJPanel1().add(getLabelSecurityExit(), gridbagconstraints16);
				...
				getJPanel1().add(getTextSecurityExit(), gridbagconstraints17);
	...
	private JTextField getTextSecurityExit() {
		if (textSecurityExit == null)
			try {
				textSecurityExit = new JTextField();
				textSecurityExit.setName("textSecurity");
				textSecurityExit.setEnabled(false);
				textSecurityExit.setColumns(20);	
			} catch (Throwable throwable) {
				handleException(throwable);
			}
		return textSecurityExit;
	}
	...
	private void oK() {
		...
		if (local) {
			...
			String tmpSecurityExit = new String(getTextSecurityExit().getText());
			if (tmpSecurityExit.trim().length() == 0) {
				this.securityExit = null;
			} else {
				this.securityExit = tmpSecurityExit;
			}
	...
	public void setSecurityExit(String value) {
		...
		this.getTextSecurityExit().setText(securityExit);
		logger.info("setSecurityExit():" + securityExit);
	}
	...
	private JLabel labelSecurityExit;
	...
	private JTextField textSecurityExit;
