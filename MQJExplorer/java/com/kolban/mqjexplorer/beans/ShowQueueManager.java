...
public class ShowQueueManager extends JDialog {
	...
	private void connEtoM7(ItemEvent itemevent) {
		try {
			getJLabel4().setEnabled(getShowRemoteRadioButton().isSelected());
		} catch (Throwable throwable) {
			handleException(throwable);
		}
	}
		...
		try {
			if (!ivjConnPtoP1Aligning) {
				...
				if (getLocalQueueManagerDocument() != null)
					getLocalQueueManagerName().setDocument(
							getLocalQueueManagerDocument());
				ivjConnPtoP1Aligning = false;
			}
		} catch (Throwable throwable) {
			ivjConnPtoP1Aligning = false;
	...
	private void connPtoP1SetTarget() {
		try {
			if (!ivjConnPtoP1Aligning) {
				ivjConnPtoP1Aligning = true;
				setLocalQueueManagerDocument(getLocalQueueManagerName()
						.getDocument());
				ivjConnPtoP1Aligning = false;
			}
		} catch (Throwable throwable) {
	...
	private JLabel getJLabel4() {
		if (ivjJLabel4 == null)
			try {
				...
				ivjJLabel4.setText("Channel Name:");
	...
	private JPanel getJPanel1() {
		if (ivjJPanel1 == null)
			try {
				...
				getJPanel1().add(getRemoteQueueManagerName(),
						gridbagconstraints5);
	...
	private JTextField getRemoteChannelName() {
		if (ivjRemoteChannelName == null)
			try {
				...
				ivjRemoteChannelName.setName("RemoteChannelName");
