...
import com.kolban.swing.ColumnSelector2;
import com.kolban.swing.SwingUtils;
import com.kolban.swing.TableMetaData;
import com.kolban.swing.table.HeaderListener;
import com.kolban.swing.table.SortButtonRenderer;
import com.kolban.swing.table.SortableTableModel;
...
public class QueueView extends JPanel implements MQJExplorerView, ActionListener, MouseListener {
	...
	public void actionPerformed(ActionEvent actionevent) {
		...
		if (s.equals("showSystem")) {
			queueManagerModel.setShowSystemQueues(showSystemToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		if (s.equals("showTempQueues")) {
			queueManagerModel.setShowTempQueues(showTempQueuesToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		if (s.equals("showAliasQueues")) {
			queueManagerModel.setShowAliasQueues(showAliasQueuesToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		if (s.equals("showModelQueues")) {
			queueManagerModel.setShowModelQueues(showModelQueuesToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		if (s.equals("showRemoteQueues")) {
			queueManagerModel.setShowRemoteQueues(showRemoteQueuesToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		if (s.equals("showLocalQueues")) {
			queueManagerModel.setShowLocalQueues(showLocalQueuesToggleButton
					.isSelected());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			return;
		}
		...
		if (s.equals("depleteMessages")) {
			...
			logger.info("** Uncaught command: " + s);
			return;
		}
	}

	public void addToolBarButtons(JToolBar jtoolbar, Dimension dimension) {
		logger.trace("addToolBarButtons()");
		showLocalQueuesToggleButton = new JToggleButton(new ImageIcon(
				getClass().getResource("/icons/localQueues.gif")));
		...
		showRemoteQueuesToggleButton = new JToggleButton(new ImageIcon(
				getClass().getResource("/icons/remoteQueues.gif")));
		...
		showModelQueuesToggleButton = new JToggleButton(new ImageIcon(
				getClass().getResource("/icons/modelQueues.gif")));
		...
		showAliasQueuesToggleButton = new JToggleButton(new ImageIcon(
				getClass().getResource("/icons/aliasQueues.gif")));
		...
		showTempQueuesToggleButton = new JToggleButton(new ImageIcon(getClass()
				.getResource("/icons/tempQueues.gif")));
		...
		showSystemToggleButton = new JToggleButton(new ImageIcon(getClass()
				.getResource("/icons/systemObjects.gif")));
		...
		if (queueManagerModel == null) {
			...
			showAliasQueuesToggleButton.setSelected(queueManagerModel
					.isShowAliasQueues());
			showLocalQueuesToggleButton.setSelected(queueManagerModel
					.isShowLocalQueues());
			showModelQueuesToggleButton.setSelected(queueManagerModel
					.isShowModelQueues());
			showRemoteQueuesToggleButton.setSelected(queueManagerModel
					.isShowRemoteQueues());
			showTempQueuesToggleButton.setSelected(queueManagerModel
					.isShowTempQueues());
			showSystemToggleButton.setSelected(queueManagerModel
					.isShowSystemQueues());
		}
	}
	...
	public void browse() {
		...
		if (s == null)
			return;
		logger.info("Browsing queue: " + s);
		try {
			MessageListDlg messagelistdlg = new MessageListDlg(
					MQJExplorer.mainFrame, queueManagerModel.getQueueManager(),
					s);
	...
	public void clearSelectedQueue() {
		logger.trace("clearSelectedQueue()");
		int i = table.getSelectedRow();
		if (i < 0)
			return;
		try {
			PCFUtils.clearQueue(queueManagerModel.getAgent(),
	...
	public void consumeMessages() {
		try {
			Consumer consumer = new Consumer(MQJExplorer.mainFrame,
					queueManagerModel.getNewQueueManager(),
					getSelectedQueueName());
	...
	public void copyQueue() {
		...
		if (copyqdlg.getResult() == 1) {
			try {
				queueManagerModel.getQueueListModel().refreshQueue(
						queueManagerModel.getAgent(),
						copyqdlg.getNewQueueName());
			} catch (Exception _ex) {
			}
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
		}
	...
	public void createQueue(int i) {
		try {
			...
			case 6: // '\006'
				...
				newremotequeue
						.doModal(MQJExplorer.mainFrame, queueManagerModel);
			...
			case 5: // '\005'
			default:
				logger.info("Unexpected queue type: " + i);
				return;
			}
			queueManagerModel.getQueueListModel().refreshQueue(
					queueManagerModel.getAgent(), s);
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
		} catch (Exception _ex) {
		}
	}
	...
	public void deleteSelectedQueue() {
		...
		if (i < 0)
			return;
		try {
			Object aobj[] = { "Yes - With purge", "Yes - No purge", "Cancel" };
			int j = JOptionPane.showOptionDialog(this,
					"Confirm deletion of this queue?", "Confirm deletion", 1,
					3, null, aobj, aobj[2]);
			boolean flag = false;
			switch (j) {
			case 2: // '\002'
			...
			if (mqexception.reasonCode == 2055) {
				int k = JOptionPane.showConfirmDialog(this,
						"Queue contains messages, empty and delete?",
						"Empty and delete?", 0, 3);
				if (k == 0)
					try {
	...
	public void enableQueue(int i) {
		...
		try {
			...
			logger.error("Exception while enabling queue: "
	...
	private JMenuItem getJMenuItem1() {
		if (ivjJMenuItem1 == null)
			try {
				...
				ivjJMenuItem1.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Delete16.gif")));
	...
	private JMenuItem getJMenuItem10() {
		if (ivjJMenuItem10 == null)
			try {
				...
				ivjJMenuItem10.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Down16_1.gif")));
	...
	private JMenuItem getJMenuItem12() {
		if (ivjJMenuItem12 == null)
			try {
				...
				ivjJMenuItem12.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Up16_1.gif")));
	...
	private JMenuItem getJMenuItem13() {
		if (ivjJMenuItem13 == null)
			try {
				...
				ivjJMenuItem13.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Delete16.gif")));
	...
	private JMenuItem getJMenuItem131() {
		if (ivjJMenuItem131 == null)
			try {
				...
				ivjJMenuItem131.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Delete16.gif")));
	...
	private JMenuItem getJMenuItem14() {
		if (ivjJMenuItem14 == null)
			try {
				...
				ivjJMenuItem14.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/Copy.gif")));
	...
	private JMenuItem getJMenuItem2() {
		if (ivjJMenuItem2 == null)
			try {
				...
				ivjJMenuItem2.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menu_time.gif")));
	...
	private JMenuItem getJMenuItem3() {
		if (ivjJMenuItem3 == null)
			try {
				...
				ivjJMenuItem3.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem31() {
		if (ivjJMenuItem31 == null)
			try {
				...
				ivjJMenuItem31.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem311() {
		if (ivjJMenuItem311 == null)
			try {
				...
				ivjJMenuItem311.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem4() {
		if (ivjJMenuItem4 == null)
			try {
				...
				ivjJMenuItem4.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem41() {
		if (ivjJMenuItem41 == null)
			try {
				...
				ivjJMenuItem41.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem411() {
		if (ivjJMenuItem411 == null)
			try {
				...
				ivjJMenuItem411.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Refresh16.gif")));
	...
	private JMenuItem getJMenuItem5() {
		if (ivjJMenuItem5 == null)
			try {
				...
				ivjJMenuItem5.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/qminus_blue.gif")));
	...
	private JMenuItem getJMenuItem6() {
		if (ivjJMenuItem6 == null)
			try {
				...
				ivjJMenuItem6.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/Find16.gif")));
	...
	private JMenuItem getJMenuItem7() {
		if (ivjJMenuItem7 == null)
			try {
				...
				ivjJMenuItem7.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/qplus_blue.gif")));
	...
	private JMenuItem getJMenuItem71() {
		if (ivjJMenuItem71 == null)
			try {
				...
				ivjJMenuItem71.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/qplus_blue.gif")));
	...
	private JMenuItem getJMenuItem8() {
		if (ivjJMenuItem8 == null)
			try {
				...
				ivjJMenuItem8.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Properties16.gif")));
	...
	private JMenuItem getJMenuItem81() {
		if (ivjJMenuItem81 == null)
			try {
				...
				ivjJMenuItem81.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Properties16.gif")));
	...
	private JMenuItem getJMenuItem811() {
		if (ivjJMenuItem811 == null)
			try {
				...
				ivjJMenuItem811.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Properties16.gif")));
	...
	private JMenuItem getJMenuItem9() {
		if (ivjJMenuItem9 == null)
			try {
				...
				ivjJMenuItem9.setIcon(new ImageIcon(getClass().getResource(
						"/icons/menuicons/qminus_blue.gif")));
	...
	public static void main(String args[]) {
		try {
			...
			jframe.setSize(jframe.getWidth() + insets.left + insets.right,
					jframe.getHeight() + insets.top + insets.bottom);
	...
	public void putAdvanced() {
		...
		try {
			...
			logger.info("QueueView::putAdvanced "
					+ mqexception.toString());
			QueueManagerModel.displayError(mqexception);
		}
	}
	...
	public void putTest() {
		...
		puttestmessage.setQueueManagerNameText(queueManagerModel
				.getQueueManagerName());
			...
			if (mqexception.reasonCode == 2051)
				JOptionPane
						.showMessageDialog(
								...
								"Queue is put inhibited. Can not put messages to this queue.",
								"MQJExplorer", 2);
			logger.info("putTest: " + mqexception.toString());
		}
	}
	...
	public void readMessagesFromFile() {
		try {
			ReadMessagesFromFile readmessagesfromfile = new ReadMessagesFromFile(
					MQJExplorer.mainFrame, getSelectedQueueName(),
					queueManagerModel.getQueueManager());
	...
	public void refresh() {
		try {
			int i = table.getSelectedRow();
			queueManagerModel.getQueueListModel().refreshAll(
					queueManagerModel.getAgent());
			((AbstractTableModel) table.getModel()).fireTableDataChanged();
			if (i >= 0)
				((AbstractTableModel) table.getModel()).fireTableRowsUpdated(i,
						i);
	...
	public void showSelectedProperties() {
		...
		case 1: // '\001'
			...
			propertieslocalqueue.doModal(MQJExplorer.mainFrame,
					getSelectedQueueName(), queueManagerModel);
			break;

		case 3: // '\003'
			PropertiesAliasQueue propertiesaliasqueue = new PropertiesAliasQueue();
			propertiesaliasqueue.doModal(MQJExplorer.mainFrame,
					getSelectedQueueName(), queueManagerModel);
			break;

		case 2: // '\002'
			PropertiesModelQueue propertiesmodelqueue = new PropertiesModelQueue();
			propertiesmodelqueue.doModal(MQJExplorer.mainFrame,
					getSelectedQueueName(), queueManagerModel);
			break;

		case 6: // '\006'
			PropertiesRemoteQueue propertiesremotequeue = new PropertiesRemoteQueue();
			propertiesremotequeue.doModal(MQJExplorer.mainFrame,
					getSelectedQueueName(), queueManagerModel);
			break;
		}
		refreshSelectedQueue();
