...
public class DOMTree extends JTree {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 356649568600279615L;
	...
	static class Model extends DefaultTreeModel implements Serializable {
		...
		private MutableTreeNode insertNode(String s,
				MutableTreeNode mutabletreenode) {
			DefaultMutableTreeNode defaultmutabletreenode = new DefaultMutableTreeNode(
					s);
			insertNodeInto(defaultmutabletreenode, mutabletreenode,
					mutabletreenode.getChildCount());
			return defaultmutabletreenode;
		}

		private MutableTreeNode insertDocumentNode(Node node,
				MutableTreeNode mutabletreenode) {
			MutableTreeNode mutabletreenode1 = insertNode(
					"<" + node.getNodeName() + '>', mutabletreenode);
			nodeMap.put(mutabletreenode1, node);
			return mutabletreenode1;
		}

		private MutableTreeNode insertElementNode(Node node,
				MutableTreeNode mutabletreenode) {
			...
			stringbuffer.append('>');
			MutableTreeNode mutabletreenode1 = insertNode(
					stringbuffer.toString(), mutabletreenode);
			nodeMap.put(mutabletreenode1, node);
		...
		private MutableTreeNode insertTextNode(Node node,
				MutableTreeNode mutabletreenode) {
			String s = node.getNodeValue().trim();
			if (s.length() > 0) {
				MutableTreeNode mutabletreenode1 = insertNode(s,
						mutabletreenode);
				nodeMap.put(mutabletreenode1, node);
				return mutabletreenode1;
			} else {
		...
		private MutableTreeNode insertCDataSectionNode(Node node,
				MutableTreeNode mutabletreenode) {
			StringBuffer stringbuffer = new StringBuffer();
