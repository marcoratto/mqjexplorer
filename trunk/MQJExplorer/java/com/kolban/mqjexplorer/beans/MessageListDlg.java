...
import com.kolban.mqjexplorer.MessageListTableModel;
import com.kolban.mqjexplorer.Profile;
import com.kolban.mqjexplorer.TextIconRenderer;
...
public class MessageListDlg extends JDialog {
	...
	public void columns_ActionEvents() {
		...
		columnselector2.setTable(getMessageListTable(),
				profile.getMessageListTableMetaData());
	...
	private JButton getColumns() {
		if (ivjColumns == null)
			try {
				...
				ivjColumns
						.setToolTipText("Alter the visible columns in the dialog");
	...
	private JPanel getJDialogContentPane() {
		if (ivjJDialogContentPane == null)
			try {
				...
				getJDialogContentPane().add(getScrollPane(),
						gridbagconstraints1);
	...
	private JMenuItem getJMenuItem1() {
		if (ivjJMenuItem1 == null)
			try {
				...
				ivjJMenuItem1.setIcon(new ImageIcon(this.getClass()
						.getResource("/icons/Quetion16.gif")));
	...
	private JTable getMessageListTable() {
		if (ivjMessageListTable == null)
			try {
				...
				getScrollPane().setColumnHeaderView(
						ivjMessageListTable.getTableHeader());
				...
				ivjMessageListTable
						.setPreferredScrollableViewportSize(new Dimension(500,
								300));
				ivjMessageListTable.setDefaultRenderer(
						com.kolban.mqjexplorer.TextIcon.class,
						new TextIconRenderer());
	...
	private JButton getProperties() {
		if (ivjProperties == null)
			try {
				...
				ivjProperties
						.setToolTipText("Display the properties of an individual message");
	...
	private JButton getRefresh() {
		if (ivjRefresh == null)
			try {
				...
				ivjRefresh
						.setToolTipText("Refresh the messages from the queue");
	...
	private JMenuItem getSaveToFile() {
		if (ivjSaveToFile == null)
			try {
				...
				ivjSaveToFile.setIcon(new ImageIcon(getClass().getResource(
						"/icons/Save16.gif")));
				ivjSaveToFile.setSelectedIcon(new ImageIcon(getClass()
	...
	private void maybePopup(MouseEvent mouseevent) {
		...
		if (i < 0) {
			...
			getMessagePopup().show(mouseevent.getComponent(),
