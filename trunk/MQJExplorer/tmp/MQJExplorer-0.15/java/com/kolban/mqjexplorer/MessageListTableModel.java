...
public class MessageListTableModel extends AbstractTableModel {
	...
	public Object getValueAt(int i, int j) {
		...
		if (s.equals("Put Date/Time")) {
			...
			return simpledateformat.format(((MQMD) (mqmessage)).putDateTime
					.getTime());
		}
		...
		if (s.equals("Accounting Token"))
			return StringUtils
					.byteArrayToHexString(((MQMD) (mqmessage)).accountingToken);
		...
		if (s.equals("Application Type"))
			...
			default:
				return "Unknown: "
						+ Integer
								.toString(((MQMD) (mqmessage)).putApplicationType);
			}
		...
		if (s.equals("Correlation ID Bytes"))
			return StringUtils
					.byteArrayToHexString(((MQMD) (mqmessage)).correlationId);
		...
		if (s.equals("Group ID Bytes"))
			return StringUtils
					.byteArrayToHexString(((MQMD) (mqmessage)).groupId);
		...
		if (s.equals("Message ID Bytes"))
			return StringUtils
					.byteArrayToHexString(((MQMD) (mqmessage)).messageId);
		...
		if (s.equals("Put Date (GMT)")) {
			SimpleDateFormat simpledateformat1 = new SimpleDateFormat(
					"M/dd/yyyy");
			return simpledateformat1.format(((MQMD) (mqmessage)).putDateTime
					.getTime());
		}
		if (s.equals("Put Time (GMT)")) {
			SimpleDateFormat simpledateformat2 = new SimpleDateFormat(
					"h:mm:ss aa");
			return simpledateformat2.format(((MQMD) (mqmessage)).putDateTime
					.getTime());
		}
	...
	public void refreshAll(MQQueueManager mqqueuemanager, String s) {
		try {
			...
			try {
				...
				logger.info("Ended mqget: " + mqexception1.toString());
			}
			mqqueue.close();
	...
	private static final String columnNames[] = { "Position", "Put Date/Time",
			"User Identifier", "Put Application Name", "Format", "Data Length",
			"Message Data", "Accounting Token", "Application Identity Data",
			"Application Origin Data", "Application Type", "Backout Count",
			"Coded Character Set ID", "Correlation ID", "Correlation ID Bytes",
			"Encoding", "Expiry", "Feedback", "Group ID", "Group ID Bytes",
			"Logical Sequence Number", "Message Data Bytes", "Message ID",
