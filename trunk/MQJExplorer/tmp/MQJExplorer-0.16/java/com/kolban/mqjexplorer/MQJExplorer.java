...
import com.kolban.swing.ConsoleLog;
import com.kolban.swing.FileTail;
...
public class MQJExplorer extends JFrame implements MQSeriesTreeListener {
	...
	private final static Logger logger = Logger.getLogger(MQJExplorer.class);
	...
	private JButton getDefaultToolBarButton() {
		if (ivjDefaultToolBarButton == null)
			try {
				...
				ivjDefaultToolBarButton.setIcon(new ImageIcon(getClass()
						.getResource("/icons/Save16.gif")));
	...
	private JPanel getJFrameContentPane() {
		logger.trace("getJFrameContentPane()");
	...
	private JButton getJToolBarButton1() {
		logger.trace("getJToolBarButton1()");
		if (ivjJToolBarButton1 == null)
			try {
				...
				ivjJToolBarButton1.setIcon(new ImageIcon(getClass()
						.getResource("/icons/Refresh16.gif")));
	...
	private JButton getJToolBarButton2() {
		logger.trace("getJToolBarButton2()");
		if (ivjJToolBarButton2 == null)
			try {
				...
				ivjJToolBarButton2.setIcon(new ImageIcon(getClass()
						.getResource("/icons/selectColumns.gif")));
	...
	private JMenuBar getMQJExplorerJMenuBar() {
		logger.trace("getMQJExplorerJMenuBar()");
	...
	private JToolBar getToolBar() {
		...
		if (ivjToolBar == null)
			try {
				...
				getToolBar().add(getJToolBarButton1(),
						getJToolBarButton1().getName());
				getToolBar().add(getJToolBarButton2(),
						getJToolBarButton2().getName());
	...
	private void initialize() {
		...
		javax.swing.UIManager.LookAndFeelInfo alookandfeelinfo[] = UIManager
		...
		mqseriesTree = Profile.getInstance().getTree();
		jtree = mqseriesTree.buildTree();
		mqseriesTree.addMQSeriesTreeListener(this);
		setSize(Profile.getInstance().commonSettings.windowSize);
		...
		pubSubView.setProfile(Profile.getInstance());
		...
		splitPane.setDividerLocation(Profile.getInstance().commonSettings.dividerPosition);
		buildToolbar();
		...
		Profile.getInstance().load(Constants.XML_CONFIG_FILE);
	}
		...
		try {
			...
			mainFrame.addWindowListener(new WindowAdapter() {
				...
				public void windowClosing(WindowEvent windowevent) {
					MQJExplorer mqjexplorer1 = (MQJExplorer) windowevent
							.getWindow();
					mqjexplorer1.exit();
				}
			...
			mainFrame.setLocation(
					dimension.width / 2 - mainFrame.getWidth() / 2,
					dimension.height / 2 - mainFrame.getHeight() / 2);
			mainFrame.toBack();
			mainFrame.setVisible(true);
			splash.toFront();
			mainFrame.postStartup();
		} catch (Throwable t) {
			t.printStackTrace();
			logger.error(t);
		}
	...
	public void MQErrorDialog(MQException mqexception) {
		JOptionPane.showMessageDialog(this, "MQSeries error: "
				+ mqexception.reasonCode, "MQSeries Error", 0);
	}

	public void postStartup() {
		logger.trace("postStartup()");
	...
	public void save() {
		logger.trace("save()");
		Profile.getInstance().commonSettings.windowSize = getSize();
		Profile.getInstance().commonSettings.dividerPosition = splitPane.getDividerLocation();		
		Profile.getInstance().save(Constants.XML_CONFIG_FILE, this.jtree);
		statusBar.setMessageText("Settings saved on " + Constants.XML_CONFIG_FILE.getAbsolutePath().toString());
	}
